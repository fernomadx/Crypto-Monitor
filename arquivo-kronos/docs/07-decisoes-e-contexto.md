...
- **Candle Dynamics backtest BTC 5y (2026-09)** — formalização automática das regras de retração/impulsão/falha + gap + hierarquia M5/M30/H1/D/W. Dados Binance Vision + fill MEXC. Resultado: **INVALIDATED** no BTC spot (WR ~19%, PF < 1). Código em `lib/candle_dynamics/` + `vps/candle_dynamics_backtest.py`.

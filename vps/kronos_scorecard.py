...
def run(force_report: bool = False) -> None:
    ...
    try:
        snap = write_scorecard_snapshot(body)
        logger.info("Scorecard snapshot: %s", snap)
    except Exception as exc:
        logger.warning("Scorecard snapshot falhou: %s", exc)

    if not short_closed and not weekly:
        logger.info("Scorecard: nada novo — snapshot atualizado, sem Telegram")
        return
    ...
    if short_closed:
        ...
        lev = os.environ.get("KRONOS_LEVERAGE", "3")
